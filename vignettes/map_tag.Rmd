...
The packages you will use are __map__, __ggplot2__ and __dplyr__. __map__ contains the function `map_tag()`, and __ggplot2__ and __dplyr__ provide powerful tools for vizualizing and wrangling data. (These packages are part of the tidyverse (https://www.tidyverse.org/) -- a collection of R packages designed for data science. All packages share an underlying philosophy and common APIs.)

```{r sho-chunk-that-was-hidden-above}
<<pkg-hide>>
```

From now on, The data you will use is an example data set from Barro Colorado Island made public in 2012 (see https://repository.si.edu/handle/10088/20925 and [`?bciex::bci12vft_mini`](https://forestgeo.github.io/map/reference/bci12vft_mini.html)).
...
The second way is to use `header_map_tag()` (see `?header_map_tag()`).

```{r header-via-helper}
map_tag(bci_vft1_rnm, title_quad = "BCI 2012", header = header_map_tag())[1]
```
...
```{r}
map_tag(bci_vft1_rnm, title_quad = "BCI 2012", extend_grid = 0.4)[4]
```
