...
some_site_vft <- map::bci12vft_mini %>%
...
The packages you will use are __map__, __ggplot2__ and __dplyr__. __map__ contains the function `map_tag()` (after I receive some feedback I will move it to the package __forestr__). __ggplot2__ and __dplyr__ provide powerful tools for vizualizing and wrangling data. (These packages are part of the tidyverse (https://www.tidyverse.org/) -- a collection of R packages designed for data science. All packages share an underlying philosophy and common APIs.)

```{r sho-chunk-that-was-hidden-above}
<<pkg-hide>>
```

From now on, The data you will use is an example data set from Barro Colorado Island made public in 2012 (see https://repository.si.edu/handle/10088/20925 and [`?map::bci12vft_mini`](https://forestgeo.github.io/map/reference/bci12vft_mini.html)).
