...
prepared <- splitted %>%
  ...
  purrr::map(paginate) %>%
  purrr::map(rename, quadrat = quadrat_vftbl) %>%
  purrr::map(add_subquad_limits) %>%
  purrr::map(mutate,
    id = paste0("Q. ", quadrat, " SQ. ", sqds, " (p. ", subquadrat, ")")
  ) %>%
  purrr::map(select, id, subquadrat, everything()) %>%
  purrr::map(select,
...
prep_df <- prepared %>% 
  reduce(full_join) %>% 
  # Add status to tag because some points dissapear from plot but tags persist
  mutate(
    tag = case_when(
