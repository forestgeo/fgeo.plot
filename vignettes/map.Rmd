...
# `maply_quad()` works with ViewFullTable (see ?luquillo_vft_4quad)
vft <- luquillo_vft_4quad
want <- filter(vft, CensusID == 6, PlotID == 1)
p <- maply_quad(want)
...
vft_1quad <- pick_top(luquillo_vft_4quad, QuadratName)
...
```{r}
census <- fgeo.tool::pick_top(fgeo.data::luquillo_stem6_1ha, sp, 2)
