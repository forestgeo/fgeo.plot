...
sin_q20[7:8] %>% str()
  ...
  mutate(df = mutate(value, quadrat = name))
...
id_df <- function(list_df) {
  nms <- names(list_df)
  named_df_nested <- tibble::enframe(list_df)
  named_df <- tidyr::unnest(named_df)
}

list_df %>% map(id_df)


# Take a list of data frames and subset only one
filter_df_by_id <- function(df, id) {
}
