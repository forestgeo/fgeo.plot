...
```{r}
explicit <- map(sin_q20, codify_explicitely)
```
...
subtitle_string <- paste0(
  ...
  "\n",
  "\n",
  "Measuring: ___________ Measurement date: __________\n",
  "\n",
  "Recording: ___________ Checking: __________________\n",
  "\n",
  "Checked date: ________",
  "\n"
)
...
plot_repel <- function(df) {
  ...
  ggplot(df, aes(x = lx, y = ly, shape = sym2)) +
    ...
    theme + 
    labs(
      title = paste("Quadrat:", title_string), 
...
# df <- with_limits %>% filter(subquadrat == 1)
```



```{r}
ided <- identify_subquadrat(explicit) %>% as_tibble()



fifteen <- ided %>% filter(quadrat == 15) %>% as.tibble()
with_limits <- add_limits_shrinked(fifteen)




pdf(onefile = TRUE, paper = "a4", width = 8.2, height = 8.2)


with_limits %>% split(fifteen$id) %>% map(plot_repel)
