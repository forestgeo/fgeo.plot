...
Similarly to how you customized the header, you can also customize the plot theme. The argument `theme` defaults to `theme = get_theme()`, but you can either use any pre-made theme (see `?ggplot2::theme_bw`) or create a custom theme. This is how you can use a pre-made theme.
...
```{r}
map_tag(renamed, site_name = "BCI 2012", extend_grid = 0.4)[4]
```

## Customizing the Dimension of Quadrats and Subquadrats

You can customize the dimension of your quadrats and subquadrats to fit the range of `qx` and `qy` of your data. Let's examine that range is for the data you'd be using so far.

```{r}
qx_and_qy_variables <- select(renamed, qx, qy)
lapply(qx_and_qy_variables, range)
```

The range is between around 0 and 20. That is why we've been using using the default quadrat dimension of 20 meters (`x_sq` and `y_sq`), and the default subquadrat dimension of 5 meters (`x_sq` and `y_sq`).
...
# Using 
map_tag(renamed, 
  site_name = "BCI 2012", x_q = 100, x_sq = 50, 
)[1]
```

This image should flag that you have this kind of problem -- by which the range of data is smaller than quadrat and subquadrat dimensions you set. Notice that the points are all between `qx` and `qy` ranging 0-20. The tags, however, go beyond 20 because they automatically repel themselves to avoid overlapping.

## The Right Dimensions for the Right Data

If `qx` and `qy` of your data was in the 0-100 meters range, then yes, the appropriate quadrat dimension to use would be 100 and 50.
...
map_tag(bigger, x_q = 100, x_sq = 25, extend_grid = -1.75)[1]
```
...
If you only want to calculate the variable `subquadrat`, you don't need to use `map_tag()` -- can do that with `add_subquadrat()`.

```{r}
with_subquadrat <- add_subquadrat(renamed, x_q = 20, x_sq = 5)
select(
  with_subquadrat, 
...
Although it is unecessary and a bit more complicated, you can also get the `subquadrat` variable from the data the underlies the maps.

```{r}
library(purrr)

maps <- map_tag(renamed, x_q = 20, x_sq = 5)
